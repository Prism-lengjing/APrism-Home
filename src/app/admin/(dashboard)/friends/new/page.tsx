...
export default function NewFriendPage() {
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl font-bold text-foreground mb-8">添加友链</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
        ...
        <div><label className="block text-sm font-medium text-foreground mb-2">链接 URL</label><input value={form.url} onChange={e => setForm({...form, url: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-border bg-background text-foreground" required placeholder="https://..." /></div>
        <div><label className="block text-sm font-medium text-foreground mb-2">Logo URL</label><input value={form.logo} onChange={e => setForm({...form, logo: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-border bg-background text-foreground" placeholder="https://..." /></div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <button type="submit" disabled={loading} className="glass-button-primary px-8 py-3 font-medium disabled:opacity-50">{loading ? "添加中..." : "添加友链"}</button>
      </form>
    </div>
  );
}
